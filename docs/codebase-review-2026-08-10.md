...
| 项 | 处置 |
| --- | --- |
| D1 异步冲洗流水线 | **已放弃**(2026-08-18 决策):回退后评估 —— 根因高置信为 GPU STM32-DMA2D 组件与异步冲洗共用 DMA2D 引擎无仲裁(该组件源码在 Keil pack 内不可注入仲裁,自带 FIXME 亦表明不支持并发);且 D2/D3 落地后常态失效已近零,D1 的重叠收益所剩无几。维持同步忙等冲洗。 |
