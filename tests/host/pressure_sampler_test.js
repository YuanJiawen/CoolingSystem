// 运行压力采样适配器测试的 wasm 产物:输出失败数/首个失败行号,以退出码报告
'use strict';
...
if (!wasmPath) {
    console.error('usage: node pressure_sampler_test.js <test.wasm>');
    process.exit(2);
}
...
WebAssembly.instantiate(buf, {}).then(({ instance }) => {
    ...
    console.log('PASS: all pressure sampler assertions passed');
    process.exit(0);
}).catch((err) => {
