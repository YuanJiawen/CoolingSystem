...
| 事件框架(Event Framework) | ISR-safe 消息队列 + 按 event_id 分发;接口已于 2026-08-18 收缩为 init / publish_unique / register_handler / dispatch 四个(均有宿主测试,含满载覆盖与临界区配平) | event_framework/ |
...
- UI 不依赖控制模块内部,只消费**状态快照**;显示路径的压力值经**显示钳位**单源函数。
